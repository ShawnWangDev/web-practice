...
class Subject_Info:
    def __init__(self,category_name,subject_name,total_time):
...
def all_info_get_by_user_id(user_id):
    ...
    for s in subject_list:
        total=record_dao.get_total_minutes_by_subject_id_and_user_id(s.id,user_id)
        info=Subject_Info(s.category.name,s.name,total_time=total)
